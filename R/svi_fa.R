...
#' @param batch_prop Proportion of observations to be sampled in each iteration of SVI. A number between 0 and 1.
#' @param fr Forgetting rate of SVI update schedule. A non-negative number.
#' @param delay Delay of SVI update schedule. A non-negative integer.
...
#' @param scale Controls whether the data should be scaled prior to performing SVI.
...
svi_fa <- function(X,
  ...
  update_omega_phi <- function(P, J, v_s, mean_Phi, var_Phi, dist_delta){
    ...
    lapply(1:P, function(p){sapply(1:J, function(k) {
      alpha <- (3 + 1)/2
  ...
  update_Phi <- function(P, X, N, J, dist_omega_phi, dist_delta, mean_Psi, mean_f, var_f){
    ...
    mean_phi <- lapply(1:P, function(p){
      f <- matrix(unlist(mean_f), nrow=nrow(X), ncol=J, byrow=TRUE)
  ...
  while(TRUE){
    ...
    mean_f_store[minibatch] <- mean_f_batch
    ...
    Psi_params_update[[1]] <- Psi_params_opt$shape
    Psi_params_update[[2]] <- step_size * Psi_params_opt$rate + (1-step_size) * Psi_params[[2]]
    names(Psi_params_update) <- c("shape", "rate")
  ...
  # Estimate for Psi
  Psi_estimated <- Psi_params_update[[2]]/(Psi_params_update[[1]] - 1)

  # Estimate for latent factors
  l_estimated <- do.call(cbind, mean_f_store)
  l_var <- var_f
